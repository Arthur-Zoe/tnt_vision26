...
struct RotationMotionDebugState {
    bool robust_ekf_active = false;
    std::string tracker_state = "LEGACY";
    int matched_id = -1;
...
private:
    ...
    // Four-armor vehicles use the validated v4 11D robust EKF.
    // Outpost keeps the original 3-armor implementation.
    bool robust_ekf_active_ = false;
    std::shared_ptr<YAML::Node> config_file_ptr_;
    std::unique_ptr<rm_ekf::RobustArmorTracker> robust_tracker_;
    rm_ekf::TrackerResult robust_last_result_;

    void syncLegacyFieldsFromRobust();

public:
    RotationMotionModel(ObservedData& initObservedData,
                        std::shared_ptr<RestFrame> rest_frame_,
                        bool is_outpost,
                        double init_r,
                        std::shared_ptr<YAML::Node> config_file_ptr = nullptr);
    ...
    RotationMotionDebugState getDebugState() const;
